...
const hourlyForecast = computed(() => {
  ...
  return forecast.forecastday?.[0]?.hour.filter(({time_epoch}) => time_epoch >= now);
});

// Get gradient color based on time of day
const getCardColor = (isDay: number) => {
  if (isDay === 1) {
    return isDark.value?'bg-black':' bg-gradient-to-b from-orange to-orange-50';
  }
  return isDark.value?'bg-black':'bg-gradient-to-b from-indigo to-indigo-50';
};
</script>
...
<template>
  <PrimaryCard class="h-full">
    ...
    <div class="scrollbar-hover flex gap-2 overflow-x-auto ">
      <div
        v-for="({time_epoch, is_day, condition, temp_c, wind_kph,time}) in hourlyForecast"
        :key="time_epoch"
        :class="[
      ...
      >
        <p class="font-bold text-xl mb-2">{{ useTimeFormat(time) }}</p>
        <img 
