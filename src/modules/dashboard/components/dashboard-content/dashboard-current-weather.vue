        ...
        <div class="flex justify-between flex-col md:flex-row gap:3 md:gap-6  h-full">
            ...
            <div class="flex flex-col items-center justify-between text-text ">
                <div>
                    ...
                    <p v-if="isTemperatureDifferent" class="text-md md:text-lg font-semibold">Feels like: {{ formatNumber(feelslike_c) }}°C
                    </p>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div v-for="(stat, index) in weatherStats" :key="index"
                    ...
                    <p class="md:text-xl font-bold ">{{ stat.value }}</p>
                    <p class="text-md">{{ stat.label }}</p>
                </div>
            </div>
